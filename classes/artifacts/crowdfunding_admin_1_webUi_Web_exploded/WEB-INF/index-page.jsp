...
<head>
    ...
    <!--
    ①将contextPath放入application域中，
    	将context.getContextPath()返回值存入application中，属性名为appPath
    	在页面上使用${appPath}获取，路径写法：${appaPath}/xxx/xxx/xxx
   	②浏览器完整的访问地址 http://localhost:8080/crowdfunding-admin-1-webUi/index.html
    	所有域路径（包括control路径，如xxx.html）相关的，前面都会默认加入base标签的href的路径，
   		base标签必须放在head标签内部，必须放在所有带路径的标签前面
